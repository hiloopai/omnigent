...
import { getCurrentAuthorId, resolveIdentity } from "@/lib/identity";
...
// session. A `null`/absent owner (single-user mode, or a legacy row with no
// owner grant) reads as owned, matching the prior permissive-on-null stance;
// otherwise the viewer owns it iff they are the owner. `viewerId` is the
// resolved current user (see `useViewerId`), `null` until identity resolves —
// treated as "not the owner" for shared rows so they don't briefly flash into
// "My sessions" before the id lands.
...
// The current viewer's user id, resolved reactively. `getCurrentAuthorId()`
// is synchronous (populated once `resolveIdentity` has run — which `main.tsx`
// kicks off at boot), but on a cold mount it can still be null for a tick, so
// we also await `resolveIdentity()` and re-render when it lands. Keeping this
// reactive (rather than a bare module read) means the My/Shared split settles
// correctly the moment identity is known, without a manual refresh.
function useViewerId(): string | null {
  const [viewerId, setViewerId] = useState<string | null>(() => getCurrentAuthorId());
  useEffect(() => {
    let cancelled = false;
    void resolveIdentity().then(() => {
      if (!cancelled) setViewerId(getCurrentAuthorId());
    });
    return () => {
      cancelled = true;
    };
